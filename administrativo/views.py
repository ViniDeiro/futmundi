...
def custom_now():
    # Usar o tempo atual sem deslocamento
    # Removemos o ajuste de -3 horas para manter a hora real
    return original_now()
...
def futliga_jogador_salvar(request):
    ...
    """
    try:
        logger.info("Iniciando salvamento de dados de Futligas Jogadores")
        data = json.loads(request.body)
        logger.info(f"Dados recebidos: {json.dumps(data, default=str)}")
        
        # Validar estrutura básica dos dados
        if 'levels' not in data:
            logger.error("Estrutura de dados inválida: 'levels' não encontrado")
            return JsonResponse({'success': False, 'error': "Estrutura de dados inválida: 'levels' não encontrado"}, status=400)
        
        if 'prizes' not in data:
            logger.error("Estrutura de dados inválida: 'prizes' não encontrado")
        ...
        with transaction.atomic():
                    ...
                    if image_url and isinstance(image_url, str):
                        if image_url.startswith('data:image'):
                            ...
                            try:
                                ...
                                ext = format.split('/')[-1]
                                logger.info(f"Processando imagem base64 para o nível {level_data.get('name')} com formato {ext}")
                                image_data = ContentFile(base64.b64decode(imgstr), name=f'level_{level_data.get("name", "unnamed")}.{ext}')
                                image = image_data
                                logger.info(f"Imagem processada com sucesso para o nível: {level_data.get('name', 'unnamed')}")
                            except Exception as e:
            ...
            default_league, created = CustomLeague.objects.get_or_create(
                ...
                defaults={
                    ...
                    'owner_premium': True,
                    'privacy': 'public',
                    'level': 1  # Usando valor 1 (Nível 1) em vez de um objeto CustomLeagueLevel
                }
            )
            
            # Deleta prêmios existentes
            logger.info("Deletando prêmios existentes")
            prize_count_before = CustomLeaguePrize.objects.count()
            logger.info(f"Prêmios existentes antes da exclusão: {prize_count_before}")
            CustomLeaguePrize.objects.all().delete()
            
            # Cria novos prêmios
            prizes_data = data.get('prizes', [])
            logger.info(f"Recebendo dados de {len(prizes_data)} prêmios")
            
            # Mapeia níveis por nome para acesso rápido
            level_map = {level.name: level for level in created_levels}
            
            for prize_data in prizes_data:
                position = prize_data.get('position')
                values = prize_data.get('values', {})
                
                logger.info(f"Processando prêmio posição {position} com valores: {values}")
                
                # Escolher o nível com mais pontos (primeira colocação) para usar a imagem
                max_value = 0
                max_level = None
                
                for level_name, value in values.items():
                    if level_name in level_map and value > max_value:
                        max_value = value
                        max_level = level_map[level_name]
                
                # Cria o prêmio com a imagem do nível de maior valor
                try:
                    prize = CustomLeaguePrize.objects.create(
                        position=position,
                        league=default_league,
                        prize=f"Prêmio {position}º lugar"
                    )
                    
                    # Verifica se o próprio prêmio tem uma imagem
                    image_url = prize_data.get('image')
                    logger.info(f"Verificando imagem para o prêmio posição {position} - Tem imagem: {image_url is not None}")
                    ...
                    if image_url and isinstance(image_url, str):
                        if image_url.startswith('data:image'):
                            try:
                                format, imgstr = image_url.split(';base64,')
                                ext = format.split('/')[-1]
                                logger.info(f"Processando imagem base64 para o prêmio posição {position} com formato {ext}")
                                image_data = ContentFile(base64.b64decode(imgstr), name=f'prize_pos_{position}.{ext}')
                                prize.image = image_data
                                prize.save()
                                logger.info(f"Imagem processada com sucesso para o prêmio posição {position}")
                            except Exception as e:
                                logger.error(f'Erro ao processar imagem do prêmio posição {position}: {str(e)}')
                        elif image_url.startswith('/media/'):
                            ...
                            try:
                                ...
                                full_path = os.path.join(settings.MEDIA_ROOT, relative_path)
                                if os.path.exists(full_path):
                                    prize.image = relative_path
                                    prize.save()
                                    logger.info(f"Reusando imagem existente para o prêmio posição {position}")
                            except Exception as e:
                                logger.error(f'Erro ao processar URL de imagem existente para o prêmio: {str(e)}')
                    else:
                        # Se o prêmio não tem imagem própria, usa a imagem do nível com maior valor
                        if max_level and max_level.image:
                            prize.image = max_level.image
                            prize.save()
                            logger.info(f"Usando imagem do nível {max_level.name} para o prêmio {position}º lugar (como fallback)")
                    
                    logger.info(f"Prêmio para posição {position} criado com ID: {prize.id}")
                    
                    # Define valores por nível
                    for level_name, value in values.items():
                        try:
                            # Busca o nível pelo nome exato do mapa já criado
                            if level_name in level_map:
                                level = level_map[level_name]
                                logger.info(f"Nível encontrado no mapa: {level.name} (id: {level.id})")
                                
                                # Salva o valor para este nível
                                prize.set_valor_por_nivel(level, value)
                                logger.info(f"Valor {value} definido para o nível {level.name} no prêmio posição {position}")
                            else:
                                logger.warning(f"Nível {level_name} não encontrado no mapa de níveis")
                        except Exception as e:
                            logger.error(f"Erro ao definir valor para o nível {level_name}: {str(e)}")
                except Exception as e:
                    logger.error(f"Erro ao criar prêmio para posição {position}: {str(e)}")
            
            # Atualiza parâmetros de premiação usando os campos corretos
            award_config = data.get('award_config', {})
            
            # Obtém ou cria o registro de parâmetros
            params = Parameters.objects.first()
            if not params:
                params = Parameters.objects.create()
            ...
            # Atualiza os valores nos campos apropriados
            params.weekly_award_day = weekly.get('day', 'Segunda')
            
            # Trata o campo time com formato apropriado para TimeField
            weekly_time = weekly.get('time')
            if weekly_time:
                try:
                    # Se o formato for HH:MM, converte para time
                    hour, minute = weekly_time.split(':')
                    params.weekly_award_time = datetime.time(int(hour), int(minute))
                except:
                    # Se houver erro na conversão, deixa como está
                    pass
            
            params.season_award_month = season.get('month', 'Janeiro')
            params.season_award_day = season.get('day', '1')
            
            # Trata o campo time para season
            season_time = season.get('time')
            if season_time:
                try:
                    hour, minute = season_time.split(':')
                    params.season_award_time = datetime.time(int(hour), int(minute))
                except:
                    pass
...
def get_matches_by_round(request):
    ...
    if request.method == 'POST':
        ...
        print(f"DEBUG: get_matches_by_round chamado com round_id={round_id}, stage_id={stage_id}")
        ...
        try:
            ...
            if round_id and not round_id.startswith('new_'):
                ...
                print(f"DEBUG: Consulta SQL: {str(matches.query)}")
                print(f"DEBUG: Encontradas {matches.count()} partidas para a rodada {round_id}")
                
                matches_data = []
                for match in matches:
                    print(f"DEBUG: Processando match id={match.id}, home={match.home_team}, away={match.away_team}")
                    match_date_str = None
                    if match.match_date:
                    ...
                    print(f"DEBUG: Match data - home_id={home_team_id}, away_id={away_team_id}, date={match_date_str}")
                ...
                print(f"DEBUG: Retornando {len(matches_data)} partidas")
                print(f"DEBUG: Dados JSON: {json.dumps(matches_data)}")
                return JsonResponse({
                    'success': True,
                    'matches': matches_data
                })
            # Caso seja uma nova rodada (ID começa com 'new_')
                ...
                # Busca o template para saber quantos jogos criar
                stage = ChampionshipStage.objects.get(id=stage_id)
                championship = stage.championship
                template = championship.template
                
                print(f"DEBUG: Template encontrado: {template.name if template else 'Nenhum'}")
                ...
                print(f"DEBUG: Template stage encontrado: {template_stage.name if template_stage else 'Nenhum'}")
                ...
                if template_stage:
                    ...
                    print(f"DEBUG: Criando {matches_per_round} partidas vazias")
        ...
        except (ChampionshipRound.DoesNotExist, ChampionshipStage.DoesNotExist) as e:
            print(f"DEBUG: Erro ao buscar jogos: {str(e)}")
            return JsonResponse({
                'success': False,
                'message': f'Erro ao buscar jogos: {str(e)}'
            })
        except Exception as e:
